"""
Initialize the services used by the Retriever :
- configuration
- logging
- database connection
- data fetcher

Initialize the repositories used by the Retriever.
- records
- params
...
# RECORDS FETCHER & CONVERTER
#----------------------
try:
    from retriever.services.article_converter_oaidc import ArticleConverterOAIDC
    record_converter = ArticleConverterOAIDC()
except RuntimeError:
    logger.critical(
        exc_info=True,
        msg="Failed to initialize the record converter. Exiting..."
    )
    sys.exit(1)  

try:
    from retriever.services.record_fetcher import RecordFetcher
    record_fetcher = RecordFetcher(config, record_converter)
except RuntimeError:
    logger.critical(
        exc_info=True,
        msg="Failed to initialize the record fetcher. Exiting..."
    )
    sys.exit(1)
